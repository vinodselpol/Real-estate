...
function CreateListing() {
    ...
    //handle delete image from formData
    const handleDeleteImage = (e) => {
        const url = e.target.previousSibling.src
        const newUrls = formData.imageUrls.filter((imageUrl) => imageUrl !== url)
  ...
  return (
    <main className='p-3 max-w-4xl mx-auto'>
        <h1 className='text-3xl text-center font-semibold my-7'>Edit a Listing</h1>
        <form onSubmit={handleSubmit} className='flex flex-col sm:flex-row gap-4'>
            ...
            <div className='flex flex-col flex-1 gap-4'>
                ...
                <div className='flex gap-4'>
                    <input  onChange={(e)=> setFiles(e.target.files)} type="file" accept="image/*" multiple id='images' className='p-3 border border-gray-300 rounded w-full' />
...

                {
                    formData.imageUrls.length > 0 && formData.imageUrls.map((url, index) => (
                        <div  key ={url} className='flex justify-between p-3 border items-center'>
                            ...
                            <button type='button' onClick={handleDeleteImage} className='p-3 text-red-700 rounded-lg uppercase hover:opacity-75'>Delete</button>
                        </div>
                ...
                <button disabled={loading || uploading} className='uppercase p-3 bg-slate-700 text-white rounded-lg hover:opacity-95 disabled:opacity-80'>{loading ? 'Creating...' : 'Update Listing'}</button>
